...
def add(request,*args,**kwargs):
    ...
    calculated_day =abs( val4 - val1 )
    calculated_month = abs(val5 - val2 )
    calculated_year = abs(val6 - val3 )
...
def gpa(request,*args,**kwargs):
    ...
    return render(request, 'calculator.html', {'result':caculated_grade,'su':Semestal_unit,'sp':Semester_point,'result2':caculated_class})
...
def serve(request,*args,**kwargs):
    if request.method== 'POST':
            ...
            recipient_list=['oyedotuna@gmail.com','agesinkole@gmail.com'], 
            fail_silently=False, 
            auth_user=None, 
            auth_password=None, 
            connection=None, 
